/* ============================================
   SacraDigit — User Facility Booking Scripts
   (user-facility-booking.js)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const facilities = [
    {
      id: 'parish-hall',
      name: 'Parish Hall',
      desc: 'Large multi-purpose hall suitable for receptions, reunions, and parish events.',
      capacity: 200,
    ...
    },
    {
      ...
      desc: 'Intimate chapel for prayer meetings, novenas, and small group gatherings.',
      capacity: 40,
      availability: 'available',
      barColor: '#c9a84c',
    },
    {
      id: 'catechetical-a',
      name: 'Catechetical Room A',
      desc: 'Classroom-style room ideal for Bible studies, seminars, and formations.',
      capacity: 30,
    ...
    {
      id: 'catechetical-b',
      name: 'Catechetical Room B',
      desc: 'Smaller discussion room for group meetings and ministry gatherings.',
      capacity: 20,
      availability: 'available',
      barColor: '#9ca3af',
    },
    {
      ...
      desc: 'Flexible open space for youth activities, concerts, and community events.',
      capacity: 150,
      availability: 'available',
      barColor: '#16a34a',
    },
  ...
  function renderFacilityGrid() {
        ...
        <div class="facility-card-body">
          ...
          <div class="facility-meta">
            <span class="facility-capacity">
              <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a4 4 0 00-3-3.87M9 20H4v-2a4 4 0 013-3.87m6-2.13a4 4 0 10-4-4 4 4 0 004 4zm6 0a4 4 0 10-3.5-5.93"/></svg>
              Up to ${f.capacity} pax
            </span>
            <span class="facility-avail-badge ${f.availability}">
              ${f.availability === 'available' ? 'Available' : 'Limited Slots'}
            </span>
          </div>
        </div>
        <div class="facility-card-footer">
          <button type="button" class="btn-book-this" data-facility-id="${f.id}">
            Book This Facility
          </button>
        </div>
      </div>
    ...
    // "Book This Facility" shortcuts to modal with pre-selected facility
    grid.querySelectorAll('.btn-book-this').forEach(btn => {
      btn.addEventListener('click', () => {
        const fId = btn.dataset.facilityId;
        openModal(fId);
      });
    });
  }
  ...
  function openModal(preSelectFacilityId = null) {
    ...
    // Pre-select if coming from a card button
  ...
  document.getElementById('booking-submit').addEventListener('click', () => {
    ...
    let valid = true;
    [{ el: facilitySelect, val: facility },
     { el: dateInput,       val: date },
     { el: timeInput,        val: time },
     { el: purposeInput,      val: purpose }].forEach(({ el, val }) => {
      if (!val) {
