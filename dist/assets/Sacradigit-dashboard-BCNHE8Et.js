import{n as e,nt as t,t as n}from"./amplify-init-DgBa3hMn.js";/* empty css                  */import{t as r}from"./dashboard-C6eKpHew.js";var i=t((()=>{e(),document.addEventListener(`DOMContentLoaded`,()=>{let e=new Date().toISOString().slice(0,10);function t(e){return e?new Date(e).toLocaleDateString(`en-US`,{month:`short`,day:`numeric`,year:`numeric`}):`—`}let r={digitized:`badge-green`,processing:`badge-amber`,queued:`badge-gray`},i={digitized:`Digitized`,processing:`Processing`,queued:`Queued`},a=document.getElementById(`todays-schedule-list`);n.models.Mass.observeQuery({filter:{date:{eq:e}}}).subscribe({next:({items:e})=>{let t=e.slice().sort((e,t)=>e.time.localeCompare(t.time)),n=t.filter(e=>e.isSpecial).length,r=t.length-n;document.getElementById(`stat-todays-masses`).textContent=t.length,document.getElementById(`stat-todays-masses-sub`).textContent=`${r} regular · ${n} special`,a.innerHTML=t.length===0?`<li class="list-row text-sm text-gray-400">No masses today.</li>`:t.map(e=>`
            <li class="list-row">
            ...
            </tr>`).join(``)},error:e=>console.error(`Failed to load records:`,e)})})}));r(),i();